function key_huydev(req, res, next) {
  ...
  try {
    ...
    if (!huykaiserdev) {
      return res.json({
        error: 'APIKEY không chính xác liên hệ admin: https://www.facebook.com/dungkon.tick'
      });
    } else {
      if (huykaiserdev.request === 0) {
        return res.json({
          error: 201,
          message: 'APIKEY của bạn đã hết lượt request liên hệ admin: https://www.facebook.com/dungkon.tick'
        });
      } else {
