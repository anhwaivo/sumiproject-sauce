...
exports.index = async (req, res, next) => {
  try {
    ...
    if (!link) return res.status(400).json({ error: 'Vui lòng nhập link muốn cap.' });

    const response = await axios.get(`https://all.dungkon.id.vn/cap?link=${encodeURIComponent(link)}`);
